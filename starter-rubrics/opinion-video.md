...
> **如果你想试 v2.1**：跑完 5 篇 v2 校准 + 主动收集 MS 和 TS 的"试评分"作为候选维度数据。证据足够（≥3 样本支持）后通过 `/hit-bump` 走完整升级流程。

---

## Bucket 预测（**仅适用于已有粉丝基础的校准期用户**）

> ⚠️ **新人不要用这套绝对桶**。0 粉账号的视频典型在 100-1000 播放区间——照搬下表会让所有预测都落"底部 99%"。 <!-- hl-ex -->
> Cold-start 用户请用 [opinion-video-zero.md](opinion-video-zero.md) 的**比率桶方案**。
> Calibration 用户（≥5 篇有数据）由 `/hit-init` 询问 baseline 后自动派生。
> 下表是"参考博主"（中文抖音观点视频，~1w 粉，25+ 已发样本）的实际拟合——**不是普适常量**。 <!-- hl-ex -->

不要预测一个具体数字。预测一个 **bucket** + 概率分布。

| Bucket | 范围（万播放）| 参考博主的先验概率 | <!-- hl-ex -->
|---|---|---|
| 底部 | <5w | 5% | <!-- hl-ex -->
| 基础盘 | 5-30w | 35% | <!-- hl-ex -->
| 命中 | 30-100w | 45% | <!-- hl-ex -->
...
**重要**：按你的账号自己的 percentile 设 bucket 边界。你的"爆款"可能是 5w（参考博主级别）或 5k（新人级别）或 500w（大 V）。直接照抄上表的人会得到永远偏高 / 偏低的预测。 <!-- hl-ex -->

校准过 ≥5 篇后跑 `/hit-bump --bucket-only` 让系统基于你的实际分布自动派生。 <!-- hl-ex -->
...
打分新稿时，找 2-4 个 composite 接近的旧样本，列出它们的实绩。**锚点对比能抓出公式抓不到的错误**。 <!-- hl-ex -->
...
## 与 `hit-lab` 子 skill 的对应

这份 starter rubric 被以下 skill 读取：
- `/hit-init` cold-start 模式 → 默认复制本文件到 `rubric_notes.md`（如选择 calibration 模式且 mode 决定能用 v2 公式）
- `/hit-score` 和 `/hit-predict` → 读 `rubric_notes.md` 拿当前公式
- `/hit-bump` → 这份 starter 是 bump 的"先验起点"，bump 后被替换为用户自己拟合的版本
