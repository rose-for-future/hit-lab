...
跑完 5 篇（每篇都走完 `/hit-predict` → 发布 → `/hit-retro` 闭环）之后，你会有第一份个人校准数据，可以提议第一次 `/hit-bump` 升级到 v1（或直接采用 [opinion-video.md](opinion-video.md) 的 v2 作为起点重新校准权重）。
...
> ⚠️ **新人的 bucket 不能用绝对数字**——参考博主的"5w 是底部"对一个 0 粉新人来说是"现象级爆款"。 <!-- hl-ex -->
...
| Bucket | 范围（**实际播放数，不是万**）| 含义 | 先验概率 | <!-- hl-ex -->
|---|---|---|---|
| 底部 | < 100 | 几乎被算法埋了 | 30% |
...
> 公众号 / Substack 把"播放"换成"阅读"，量级类似（0 粉公众号第一篇阅读 50-500 是常态）。 <!-- hl-ex -->

**第 1 篇预测时**：选一个 bucket + 写概率分布。**很可能落"基础盘"**——这是数学事实，不是你失败。

### 第 2 篇起：用比率桶

让 `baseline = 上一篇的实际播放数`（或最近 3 篇的中位数，如果有）。 <!-- hl-ex -->
...
**比率桶的好处**：
- 100 → 1000 播放（10x） 与 5w → 50w 播放（10x） **是同一种成就**——比率桶都标"大爆" <!-- hl-ex -->
- 你的账号从 0 粉长到 10w 粉，bucket 边界自动跟着涨 <!-- hl-ex -->
- 不需要"我今天该不该过誉这个数"——倍数是绝对的

**例子**：
- 第 1 篇 480 播放（落"基础盘"）→ baseline = 480 <!-- hl-ex -->
- 第 2 篇预测 bucket = "命中"（500-1500），实际 1200 → 命中 <!-- hl-ex -->
- 第 3 篇 baseline 滚动更新 = (480 + 1200) / 2 = 840

### 第 5 篇之后：建议固定绝对桶 + 比率桶并存

跑完 5 篇后，你的 baseline 已经从 5 个数据点稳定。`/hit-status` 会主动提示：

> 你已校准 5 篇，可以基于实际分布固定绝对 bucket 边界。 <!-- hl-ex -->
> 跑 `/hit-bump --bucket-only` 自动派生。
...
- 第 5 篇之后**必须**跑 `/hit-bump --bucket-only` 重新校准
...
2. **记录 7 维评分**：复盘后每条样本变成一个 (打分, 实绩) 配对，第 5 篇时这些配对是 rubric 第一次升级的输入 <!-- hl-ex -->
...
| N≥20 | rubric 真正成为你的「爆款公式」——但你也已经摸到自己的内容直觉了 |
...
- 我的 v0 预测 vs 实际：哪个维度被验证 / 推翻 <!-- hl-ex -->
...
### 路径 A：从 v0 → v1（自己拟合权重）
跑 `/hit-bump --propose "<你的具体权重调整>"`。系统会强制全量重打 + 跨模型审核。

### 路径 B：直接采纳 v2 作为起点
把 [opinion-video.md](opinion-video.md) 的 v2 公式抄到你的 `rubric_notes.md`，**然后跑 `/hit-bump`**——bump 流程会用你的 5 篇数据验证 v2 在你账号上是否真的更优。
