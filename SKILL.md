---
name: hit-lab
description: 把内容创作变成可校准的预测循环——打分 → 盲预测 → T+3d 复盘 → 进化 rubric，适用任何能被量化（播放/阅读/收听/点击）的内容；内置一份观点视频 rubric，其他形态可借此起步。触发词："初始化"/"打分这篇"/"启动预测"/"已发布"/"复盘"/"升级 rubric"/"推荐选题"/"抓热点"/"状态"/"找对标"/"learn from"。首次使用必须先跑 /hit-init，强烈建议先用 /hit-learn-from 导入对标账号作为初始信号源。
...
| "抓热点" / "fetch trends" / "今天有什么可做的" | `/hit-trends` | trend-sources adapter 已配置（日常补充候选池） |
