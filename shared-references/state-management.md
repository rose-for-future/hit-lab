...
被所有子 skill 引用。`.hit-state.json` 是各子 skill 共享上下文的**单一来源**——任何运行时状态、累计指标、模式标记都从这里读、写回这里。
...
```
<user-content-project>/.hit-state.json
```

**绝不**放到全局 `~/.claude/` 或 hit-lab 自己的目录——一个用户可能维护多个内容项目，每个项目独立状态。
...
- 配合 [skills/hit-score-blind/SKILL.md](../skills/hit-score-blind/SKILL.md) 的 `blocked_rubric_memo` refusal_code + hit-bump Phase 5 leak guard 自检
...
- **新增 `last_prediction_self_scored: bool`**——`true` 仅当上一次 `/hit-predict` 走了 `--skip-blind` flag 或 Phase 2.5 用户选 b（信主 Claude 自估）。hit-status / SessionStart hook 据此 nag："上次预测没走 blind sub-agent，已 N 天"
- **新增 `last_self_scored_at: ISO 8601 / null`**——`last_prediction_self_scored` 触发时的时间戳；走 sub-agent 时一起清回 null
- 配合 [skills/hit-score-blind](../skills/hit-score-blind/SKILL.md) 的 channel B 隔离协议——把 contamination 跟踪从"靠 git history"升级为"靠 state 字段"
...
- **`shoots[]` 项 schema 扩展**——新增 `scripts_path`、`script_consistency`、`script_diff_pct`、`v2_prediction_written`、`script_hash_at_shoot` 字段。语义见 hit-shoot Phase 4。这些字段记录"拍后改稿是否触发 v2 预测重判"，hit-retro 据此决定读 `## 预测 v1` 还是 `## 预测 v2`
...
| 字段 | 类型 | 写入者 | 读取者 | 说明 |
|---|---|---|---|---|
| `schema_version` | string | hit-init / hit-migrate | 所有 skill | "1.1"。schema 升级时 bump；老用户由 [/hit-migrate](../skills/hit-migrate/SKILL.md) 升级。详见 [migration-protocol.md](migration-protocol.md) |
| `skill_version` | string | hit-init | 所有 skill | hit-lab 当前版本 |
| `initialized_at` | ISO 8601 | hit-init | hit-status | 首次初始化时间，永不变 |

### 模式与配置

| 字段 | 类型 | 取值 | 写入者 | 读取者 |
|---|---|---|---|---|
| `rubric_version` | string | "v0" / "v1" / "v2" / ... | hit-init / hit-bump | hit-score / hit-predict / hit-retro |
| `content_form` | enum | "opinion-video" / "long-essay" / "short-text" / "podcast" / "other" / "mixed" | hit-init | hit-predict / hit-recommend |
...
| `baseline_plays` | int / null | 用户基准播放数；首次 init 时若有抓取历史→中位数；无→null；后续 hit-retro 第 1 篇有实绩时回填 | hit-init / hit-retro / hit-bump (--bucket-only) | hit-predict（派生 bucket 边界） |
| `data_collection` | enum | "manual" / "adapter" | hit-init | hit-retro（决定 DATA_SOURCE 默认值） |
| `pool_status` | enum | "none" / "markdown" / "notion" / "sqlite" | hit-init / hit-recommend | hit-recommend / hit-status |
| `data_layer` | enum | "markdown" / "sqlite" | hit-init / md-to-sqlite.py | 所有读 predictions 的 skill |
| `hooks_installed` | bool | true / false | hit-init | hit-status（持续提示） |
| `enabled_trend_sources` | array of string | trend-source adapter 名列表（如 `["weibo-hot", "zhihu-hot"]`） | hit-init / 用户手动 | hit-trends |
| `enabled_perf_adapters` | array of string | perf-data adapter 名列表（如 `["douyin-session"]`）。空 → hit-retro 走 manual paste | hit-init / 用户手动配置后 | hit-retro |

### 累计计数

| 字段 | 类型 | 写入者 | 用途 |
|---|---|---|---|
| `calibration_samples` | int | hit-retro（每次复盘 +1） | hit-status 进度条 / hit-bump 门槛 |
| `calibration_samples_at_last_bump` | int | hit-bump | "距上次 bump 多少新样本" |

### 时间戳（last_X_at）

| 字段 | 类型 | 写入者 |
|---|---|---|
| `last_bump_at` | ISO 8601 / null | hit-bump |
| `last_bump_self_audited` | bool | hit-bump（CROSS_MODEL_AUDIT=false 时 true） |
| `last_published_at` | ISO 8601 / null | hit-publish |
| `last_published_file` | string / null | hit-publish |
| `last_retro_at` | ISO 8601 / null | hit-retro |
| `last_trends_run_at` | ISO 8601 / null | hit-trends |
| `last_trends_added_count` | int | hit-trends |
| `last_prediction_self_scored` | bool | hit-predict（`--skip-blind` 或 Phase 2.5 选 b 时 true；下次走 sub-agent 时清回 false） |
| `last_self_scored_at` | ISO 8601 / null | hit-predict（跟随 `last_prediction_self_scored` 同步） |

### 列表队列

| 字段 | 类型 | 写入者 | 读取者 | 协议 |
|---|---|---|---|---|
| `consecutive_directional_errors` | array of "high"/"low" | hit-retro（push） / hit-bump（清空） | hit-status / hit-retro 自检 | 最近 N 次复盘的偏差方向；连续 3 同向触发 bump 提议 |
| `pending_retros` | array of file path | hit-publish（push） / hit-retro（remove） | hit-status | 等待复盘的预测文件路径 |
| `shoots` | array of {video_folder, prediction_file, shot_at, ad_hoc} | hit-shoot（push） / hit-publish（remove） | hit-status / hit-recommend / SessionStart hook | 已拍未发队列。`len(shoots) = buffer count`，`buffer_days = buffer × target_publish_cadence_days` 决定颜色 |

### 会话状态

| 字段 | 类型 | 写入者 | 读取者 | 协议 |
|---|---|---|---|---|
| `in_progress_session` | object / null | hit-predict（创建） / hit-publish（清除） | hit-publish / hit-status | 见下方"in_progress_session 子结构" |
...
state_path = os.path.join(os.getcwd(), ".hit-state.json")
if not os.path.exists(state_path):
    # 不存在 = 用户没初始化，路由到 /hit-init
...
if state.get("schema_version") != LATEST_SCHEMA:
    # 不直接 raise — 提示用户跑 /hit-migrate（非阻塞）
    log_warning(f"schema 版本不匹配：state={state.get('schema_version')}, 期望={LATEST_SCHEMA}。建议跑 /hit-migrate")
...
| 字段 | 唯一写入者 | 何时写 |
|---|---|---|
| `rubric_version` | hit-init / hit-bump | init 写初值；bump 升版 |
| `baseline_plays` | hit-init / hit-retro / hit-bump (--bucket-only) | init 时若有 adapter 抓回历史→中位数；无历史→null；retro 第 1 篇有实绩→回填；bump --bucket-only→重新计算 |
| `calibration_samples` | hit-retro | 每次复盘成功落盘 +1 |
| `pending_retros` | hit-publish（push）/ hit-retro（remove） | publish 时 push 本次；retro 完成时 remove |
| `consecutive_directional_errors` | hit-retro（push）/ hit-bump（清空） | retro 判定偏差方向时 push；bump 落地时清空 |
| `in_progress_session` | hit-predict（创建）/ hit-publish（清除） | predict 写完文件时创建；publish 登记时清除 |
| `last_bump_at` | hit-bump | bump 落地时 |
...
| 症状 | 处理 |
|---|---|
| 文件不存在 | 提示"未初始化，请跑 /hit-init"，**不**自动创建 |
| JSON 解析失败 | 提示"state file 损坏：path/to/.hit-state.json"，建议手动修复或备份 + 重新 init |
| schema_version 不识别 | 提示版本号 + 建议跑 [/hit-migrate](../skills/hit-migrate/SKILL.md)。SessionStart hook 会自动检测并提示 |
| `pending_retros` 含已删除的文件 | hit-status 检测时安静移除，不报错 |
| `in_progress_session` 文件已不存在 | hit-status 检测到 → 询问用户是否清理 |
| `calibration_samples` 与 `predictions/` 实际复盘数不一致 | hit-status 报告差异。临时手改 state 即可；持续不一致是 bug，应在下个 minor 版本里加入 hit-migrate 的 reconciliation step |

---

## 与 git 的关系

`.hit-state.json` **应该**被纳入 git：
- ✅ 它是项目配置 + 累计指标的快照
- ✅ git history 提供状态演化的完整轨迹
- ✅ 多设备同步靠 git push/pull
- ❌ **不**含敏感信息（cookie / API key 应放 `.env` 或 `.hit-secrets.json`，单独 gitignore）

`.hit-cache/` 目录**不应该**被纳入 git：
- 含 `usage.jsonl`（meta-logging 钩子的本地日志）
- 含 `trends-history.jsonl`（trend 抓取的去重缓存）
- 也可能含 adapter 调试文件（如 `douyin-session-debug/`）
- 这些是设备本地状态，跨设备同步无意义

`/hit-init` 应自动在用户项目根追加（不覆盖）`.gitignore`：
...
4. SessionStart hook 检测到不一致时自动提示用户跑 `/hit-migrate`
...
如用户确实想重置：建议**删除整个 .hit-state.json + 重跑 /hit-init**——这比手改单字段安全。

---

## Confidence label 派生表（**单一真值**）

被 hit-predict / hit-status / hit-recommend / SessionStart hook 等共同使用。从 `calibration_samples` 派生，所有 skill 用同一逻辑：
