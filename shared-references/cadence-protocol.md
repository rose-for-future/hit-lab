...
被这些子 skill 引用：`hit-status`、`hit-recommend`、`hit-shoot`、`hit-publish`、SessionStart hook。
...
- 用户给数据（粘 / URL）→ `/hit-retro` 自动跑

### 周级（用户决定的"集中处理日"）

- 抓热点（`/hit-trends`）刷新候选池
- 检查 rubric bump 触发条件
...
`/hit-shoot` 把视频加进 `state.shoots`，`/hit-publish` 移除——两个事件分开使 buffer 跟踪准确。
...
用户在 hit-init 选"灵活/不固定" → buffer 监控**关闭**。SessionStart 报告只显示"已拍未发：N 条"，不显示颜色，不警戒。

---

## 选题策略（`/hit-recommend` 推 ≥ 2 个时）
...
├─ 拍 → "推荐选题" → hit-recommend 推 2 个 →
│       user 选 → /hit-seed 写 draft (cold-start) 或 user 自己写 →
│       user 改写 → script.md → user 拍 → "拍了 videos/<...>/" → hit-shoot
└─ 不拍 → 等
```
...
hit-retro 自动跑 → 写复盘段 → 检查 bump 触发条件
├─ 触发 → 提议 /hit-bump（不强制，用户决定）
└─ 未触发 → 等下个验证样本
```
...
├─ 拍 → hit-recommend 只推 v 当前 top 1 稳分 → 立即拍
└─ 接受断更风险 → user 自负，hit-status 持续提示
```
...
├─ 发 → "已发布 https://..." → hit-publish → buffer -1
├─ 复盘 → 见情况 2
└─ 拍新 → hit-recommend 拒绝："你 buffer 已 N 条，先发掉 ≤3 条再来"
```

### 情况 5：周期性集中处理日（用户主动触发）

```
user 说"抓热点" → hit-trends → 候选池更新
+ user 说"看看 rubric 是不是该升了" → hit-status 检查同向偏差累计
+ user 说"看看 rubric_notes 行数" → hit-status 健康度检查
```
...
| Skill | 节奏责任 |
|---|---|
| `/hit-init` | 问 cadence；写 `target_publish_cadence_days`；装 SessionStart hook |
| `/hit-shoot` | 把 video folder 加 state.shoots，buffer +1 |
| `/hit-publish` | 从 state.shoots 移除对应项，buffer -1 |
| `/hit-status` | 计算 buffer + 颜色，输出报告 |
| `/hit-recommend` | 按 buffer 颜色 + 选题策略给推荐 |
| `/hit-retro` | 复盘后更新 STATUS（自动 trigger /hit-status） |
| SessionStart hook | 调 /hit-status 渲染 4-6 行报告，写到 STATUS.md |

---

## 关键差异：hit-lab vs 视频分析

| 维度 | 视频分析 | hit-lab |
|---|---|---|
| Cadence 来源 | 默认日更（CADENCE.md 硬编码） | 用户自填（hit-init 问，4 档：日/隔日/周/灵活） |
