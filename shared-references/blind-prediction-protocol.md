...
被这些子 skill 引用：`hit-predict`、`hit-retro`、主 SKILL.md。
...
子 skill 在启动 `hit-predict` 前，必须自检并向用户**主动声明**：
...
唯一允许编辑预测段的场景：**纯 markdown 排版错误**（标题层级错误、列表 bullet 格式错），且用户明确说明这是格式修复。这种情况 hook 仍会阻塞，需要用户显式 bypass（手动设置环境变量 `HIT_BYPASS_IMMUTABILITY=1` 单次）——bypass 应在 git history 留痕。
...
```
scripts/<date>_<id>_<short>.md        ← pre-shoot 草稿（hit-seed 写或用户写）
predictions/<date>_<id>_<short>.md    ← immutable 预测（hit-predict 写）
videos/<date>_<id>_<short>/           ← 拍后才建（hit-shoot 创建）
...
| 场景 | 处理 |
|---|---|
| 预测文件不小心被人手编辑了预测段 | 不自动回滚（破坏更大）。下次 `hit-retro` 检测到不一致 → 在复盘段追加 `**Integrity warning**: 预测段于 <ISO timestamp> 被外部修改，无法保证盲度`，校准价值降级为"参考"，不计入 bump 校准池 |
...
盲预测是整个 hit-lab 校准循环的**唯一信号源**。一旦预测段被事后修改，所有"哪个维度被验证 / 推翻"的判断都失去基线——你不知道当初是真预测对了，还是事后改对了。
