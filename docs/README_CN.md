<h1 align="center">
  <img src="logo.svg" alt="Hit Lab" width="720">
</h1>
...
> ⚠️ **从 v0.x 升级？** `git pull` 后在你的内容项目里跑 `/hit-migrate`。**1.3 → 1.4 是 blind channel 完整性 BREAKING 修复**——拆分 `rubric_notes.md` 防止 blind sub-agent 通过白名单读到实绩。不跑迁移的话 blind 打分会持续标 `non_blind_warning`。详见 [CHANGELOG](../CHANGELOG.md) 和 [migrations/1.3-to-1.4.md](../migrations/1.3-to-1.4.md)。
...
onboarding 还会问你的**选题主线**：热点为主（追热门话题）/ 搜问题为主（挖真实用户提问，你来给解决方案——最适合教程 / 工具号）/ 都要。选"搜问题为主"会默认接入零凭据的 `community-questions` 源（WebSearch 搜 V2EX/知乎/Reddit，**无登录态 = 零封号风险**），并把候选池空时的引导指向"挖问题"而非"抓热点"。两类源始终都在——这个选择只设默认 + 侧重。
