...
#
# Silently exits if:
#   - Not in a hit-lab project (no .hit-state.json)
...
STATE_FILE="$PROJECT_DIR/.hit-state.json"

# Silently skip if not a hit-lab project
...
if ! command -v jq >/dev/null 2>&1; then
  cat <<'EOF'
[hit-lab] SessionStart: jq not installed — skipping auto status report.
Claude can still read .hit-state.json directly. Say "状态" for full status.
EOF
  exit 0
fi
...
# Strategy: hardcode current LATEST_SCHEMA here (bumped by maintainer alongside hit-init).
...
if [[ "$schema_version" != "$LATEST_SCHEMA" && "$schema_version" != "unknown" ]]; then
  schema_mismatch="⚠️  schema 版本不一致：state=${schema_version}, skill 期望=${LATEST_SCHEMA}。建议跑 /hit-migrate（非阻塞，部分新功能可能在迁移前异常）。"
elif [[ "$schema_version" == "unknown" ]]; then
  schema_mismatch="⚠️  state.schema_version 字段缺失或损坏。建议跑 /hit-status 检查文件，或备份后重 init。"
fi

# --- Detect blind-skip contamination (hit-predict --skip-blind 或 Phase 2.5 选 b 触发) ---
self_scored_warning=""
if [[ "$last_prediction_self_scored" == "true" && -n "$last_self_scored_at" ]]; then
  ...
  if [[ $self_scored_epoch -gt 0 ]]; then
    ...
    if [[ $days_since -ge 7 ]]; then
      self_scored_warning="🚨 距上次 \`--skip-blind\` 自评预测已 ${days_since} 天——校准池累计的 contamination 风险在叠加。下次 /hit-predict 走 sub-agent 即可清除此提示。"
    else
      self_scored_warning="⚠️  上次预测走了 \`--skip-blind\`（${days_since} 天前自评，未经 channel B 隔离）。下次 /hit-predict 走默认即可清除。"
    fi
  fi
fi
...
# --- Build the report ---
echo ""
echo "[hit-lab / SessionStart 状态报告]"
echo ""
echo "$buffer_label"
echo "$retro_label"
