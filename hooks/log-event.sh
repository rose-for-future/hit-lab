#!/usr/bin/env bash
#
# hit-lab / meta-logging hook
#
# Passive event recorder. Writes one JSON line per event to
# .hit-cache/usage.jsonl in the project root. Never blocks (async fire-and-forget).
#
# Used by /hit-status to compute:
#   - "distance since last bump" (count of hit-predict invocations after last_bump_at)
...
# Output: appends one line to .hit-cache/usage.jsonl

set -uo pipefail

event_type="${1:-unknown}"
cache_dir="${CLAUDE_PROJECT_DIR:-.}/.hit-cache"
