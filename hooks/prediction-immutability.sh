#!/usr/bin/env bash
#
# hit-lab / prediction-immutability hook
#
...
#   HIT_BYPASS_IMMUTABILITY=1 — single-shot bypass; logs a warning to stderr
#
...
if [[ "${HIT_BYPASS_IMMUTABILITY:-0}" == "1" ]]; then
  echo "[hit-lab] ⚠️  IMMUTABILITY BYPASS active (HIT_BYPASS_IMMUTABILITY=1)" >&2
  echo "[hit-lab] ⚠️  This should only be used for pure markdown-formatting fixes." >&2
  echo "[hit-lab] ⚠️  Bypass will be visible in git history." >&2
  exit 0
fi
...
[hit-lab] 🚫 BLOCKED: edit targets the '## 预测' / '## Prediction' section of:
  $file_path

This violates principle #1 of hit-lab: predictions are immutable.
...
What to do instead:
  ...
  • If this is a pure markdown-formatting fix (no semantic change), you can
    bypass once with: HIT_BYPASS_IMMUTABILITY=1 (logs to stderr, visible in git).
...
[hit-lab] 🚫 BLOCKED: Write would overwrite an existing prediction file:
  $file_path
