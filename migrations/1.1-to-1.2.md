...
v1.2 引入"拍后改稿触发 v2 预测重判"——hit-shoot 检测稿子 diff ≥30% 时调用 `/hit-predict — mode: v2`，在原 prediction 文件 append `## 预测 v2` 段。

为了让 hit-retro 知道"哪条预测进校准计算"，shoots[] 项必须记录是否有 v2，以及 diff 强度（用于 rubric 升级时分析"用户改稿模式"）。
...
5. **原子写**：写到 `.hit-state.json.tmp` → rename
...
不想跑 `/hit-migrate` 的用户可以**完全不动 state**——所有 skills 都用 `state.get(field, default)` 兜底，新字段会自动用默认值。但 `schema_version` 字段会一直显示 `"1.1"`，SessionStart hook 会持续提示 mismatch warning。
...
shoots[] 里的字段会在下次 hit-shoot / hit-publish 触发时按需补全。
