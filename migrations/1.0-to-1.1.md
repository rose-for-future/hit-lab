...
| 字段 | 默认值 | 写入者 |
|---|---|---|
| `typical_duration_seconds` | 240 | hit-init Q1.5 派生 |
| `target_publish_cadence_days` | null（灵活节奏） | hit-init Q1.6 派生 |
| `rubric_form_mismatch` | false | hit-init（content_form ≠ opinion-video 时 true） |
| `benchmark_status` | "none" | hit-init Phase 2.5 |
| `benchmark_name` | null | hit-learn-from |
| `benchmark_sample_count` | 0 | hit-learn-from |
| `baseline_plays` | null | hit-init / hit-retro |
| `enabled_perf_adapters` | `[]` | hit-init |
| `last_published_file` | null | hit-publish |
| `last_retro_at` | null | hit-retro |
| `pending_retros` | `[]` | hit-publish / hit-retro |
| `shoots` | `[]` | hit-shoot / hit-publish |
...
不想跑 `/hit-migrate` 的用户可以手改 `.hit-state.json`：
...
> 手改后建议跑 `/hit-status` 验证 state 文件能被所有 skill 正常读。
