...
hit-lab 的 schema 版本演进单一来源。`/hit-migrate` 读这份文件决定要跑哪些迁移。

---

## 当前 schema_version

**`1.4`** —— 由 `hit-init` Phase 3 写入新 state 文件。

下方 `LATEST_SCHEMA` 标记位被 `hit-migrate` skill 和 SessionStart hook 引用：
...
按时间顺序，每行表示一个 schema 升级。`/hit-migrate` 用此表算出从用户当前版本到 LATEST_SCHEMA 需要按顺序跑哪些 step。
...
| 1.2 | 1.3 | NO | [1.2-to-1.3.md](1.2-to-1.3.md) | 新增 `last_prediction_self_scored: bool` + `last_self_scored_at` 字段——配合 hit-score-blind sub-agent 引入的 channel B 隔离打分。`true` 表示上次预测走了 `--skip-blind`，hit-status 持续 nag |
...
- 每份必含 4 段：
  ...
  2. **WHY** — 为什么这个改动
  3. **HOW (Claude steps)** — Claude 跑 `/hit-migrate` 时按顺序执行的自然语言步骤
  4. **Manual fallback** — 如果用户不想跑 skill，手改 `.hit-state.json` 的最小指令
