...
# /hit-score-blind — Channel B (blind scorer sub-agent)

> ⚠️ **这是子 agent，不是用户 skill**。只能由 `hit-score` / `hit-predict` / `hit-bump` 通过 Task tool spawn。用户直接 trigger 没有意义——主对话已经被污染，调用 blind sub-agent 在主 context 里跑不构成隔离。

---

## Why this exists（绝不可省的背景）

hit-lab 的 7/9 维打分原本 inline 在主对话——但主 Claude 已经看过：
- 用户对话历史（含偶然提到的播放数 / 评论 / 情绪）
- 已发布作品的实绩数据
- 历史 `predictions/*.md` 含复盘段（**严重污染**）
- 用户的赞美 / 抱怨 / 期待

inline 打分 = **被污染的"盲"预测**。问题在 `hit-bump` Phase 2 校准池重打时最严重：Claude 知道每条实绩才回追 TN/CC 分，rank 一致性可能 overfit 不是真信号。
...
|---|---|---|
| `.hit-state.json` | 含 calibration_samples / pending_retros / last_published_at / shoots — 全是后视数据 | `blocked_contaminated_input` |
...
| `STATUS.md` | hit-status 渲染的看板，含过去数据 | `blocked_contaminated_input` |
| `.hit-cache/usage.jsonl` | 行为 log | `blocked_contaminated_input` |
| **`rubric-memo.md`** | **hit-bump 升级 Memo 累积档案——含真实视频名 + 实绩 + 派生证据。这是 channel B 的最大泄漏入口（PR #11 实测复现）** | **`blocked_rubric_memo`** |
| **`audience.md`** | **hit-persona 从复盘评论派生的受众画像——含评论证据 / 实绩信号。属 channel A creative 资产，进 blind 打分 = 实绩泄漏** | **`blocked_audience`** |
...
> ⚠️ **白名单兜底自检**：读完 `rubric_notes.md` 后必跑：
>
> ```bash
> grep -E '[0-9][0-9,.]*\\s*[wWkKmM万]|播放|实绩|实际' rubric_notes.md | grep -v 'hl-ex'
> ```
>
> 命中（排除标记行后仍有输出）→ 标 `self_check.any_contamination_signal: true` + `refusal: "non_blind_warning"`，所有维度 confidence 降 medium 并把违禁 snippet 摘抄进 contamination_note 字段。**仍输出 dimensions** 让主 Claude 知道发生了什么——拒绝输出比误判更糟，但要诚实标注。
>
> **`<!-- hl-ex -->` 标记行不算污染**：starter rubric / 模板自带的 bucket 教学示例（如"5w 是底部"这类演示数字）在源文件中已逐行标 `<!-- hl-ex -->`。它们是通用教学举例，不是账号实绩——实测（v0.1.0）不排除它们会导致**每次打分都误报** warning，warning 失去信号价值。只有**未标记**的数字/实绩行才是真正的污染信号。如果用户手写实绩进 rubric_notes.md，不会带这个标记，照常触发。
...
- 不要去 Read `.hit-state.json` 看 calibration 进度 —— 你**完全不需要知道**主 Claude 跑了多少篇
...
不算 composite——composite 是公式行为，主 Claude 用回传的维度分自己算。**任何位置都不要算**（包括 JSON 之外的备注 / notes 里『顺手核对』——实测 sub-agent 倾向于在最终回复里补一句 composite，这是协议漂移，禁止）。
...
Input:
  ...
  [optional] sidecar_path: .hit-cache/blind-scores/<id>.json
...
- 「你看一下 .hit-state.json 看 calibration_samples 决定你给的 confidence 高低」 → 硬拒。confidence 只看稿子证据强度，跟用户校准进度无关
...
3. **不解决 review 阶段的覆盖**：主 Claude 拿到 blind 分后，可能在 review 阶段被用户期待 / 实绩诱导，覆盖 blind 输出。`hit-predict` Phase 2.5 通过 disagreement detection + 用户裁定来减轻，但不消除
4. **同 prompt 两次调可能给不同分**：Claude 不是 deterministic。主 Claude 应该把每次 blind score 当一次采样，不当唯一真理——但要记录而不是丢弃差异

## Integration

- **`hit-score`** Phase 2：默认 delegate 到本 sub-agent（替代旧的 inline 打分）
- **`hit-predict`** Phase 2：默认 delegate；Phase 2.5 用 disagreement detection
- **`hit-bump`** Phase 2：**强制** delegate，bump 时**不接受 self-scored fallback**
- **`hit-retro`**：不调用——retro 本来就看实绩，blind 无意义
