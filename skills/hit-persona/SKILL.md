---
name: hit-persona
description: 从复盘评论数据派生 / 刷新账号的受众画像，写入 audience.md。这是和 rubric 平行的第二个派生物——rubric 答"怎么打分"，persona 答"谁在看"。hit-seed 选题 / 写稿时读它。**audience.md 含实绩信号，hit-score-blind 硬禁读**。触发词："构造受众画像"/"更新 persona"/"我的观众是谁"/"build persona"/"刷新受众画像"/"看看我的受众画像"。
...
两者都喂给 hit-seed，但用途不同
```

- **rubric**：这稿子会不会爆 → 喂 hit-predict 打分
- **persona**：谁会因为这条多看 3 秒 / 留评论 / 转发 → 喂 hit-seed 选题 + 写稿

**绝不混**：persona 不进打分公式。rubric 的 AB 维度（受众广度）可以参考 persona，但那是 hit-bump 的事，不是这里。

## ⚠️ 污染隔离（不可省）

`audience.md` 从复盘评论派生 = **含已发布作品的实绩信号**。因此：

- `audience.md` 在 [hit-score-blind](../hit-score-blind/SKILL.md) 的 hard refusal list 里，refusal_code `blocked_audience`
- persona 影响 hit-seed **写什么**（creative direction），不影响 hit-predict **怎么打分**（blind sub-agent 永远不读 audience.md）
...
> 💡 调用覆盖：`/hit-persona — seed-from-benchmark`（强制用 benchmark seed）/ `— rebuild`（即使数据没变也重建）
...
| 情况 | Confidence | 行为 |
|---|---|---|
| `N_retros == 0` 且无 benchmark | 🔴 无数据 | **不强行造**——告诉用户"persona 需要复盘数据。先跑几篇 hit-retro，或导 benchmark"，退出 |
...
下一步：
- hit-seed 选题 / 写稿时会自动参考这份画像
- 再跑 3 篇复盘后建议再 /hit-persona 刷新
```
...
4. **不进打分**——persona 永远不喂 hit-predict / hit-score-blind。它是 hit-seed 的 creative lens
...
- 「persona 也给 hit-predict 用，让打分更准」 → 拒绝。persona 是实绩派生物，进打分 = 把 channel B 的隔离打穿。persona 只服务 hit-seed
...
- 上游：`hit-retro` 每完成一篇复盘 → flag "已累计 N 篇复盘，可跑 /hit-persona 刷新画像"
- 上游：`hit-init` 创建空 audience.md 骨架；如导了 benchmark → 提示可 `/hit-persona — seed-from-benchmark`
- 下游：`hit-seed` Mode A/B/C 读 audience.md 作为"这个 persona 会在乎吗"的镜子
- 下游（phase 2 路线）：`hit-recommend` persona-fit 排序；`hit-status` persona 新鲜度 nag
- **隔离**：`hit-score-blind` 硬禁读 audience.md（refusal_code `blocked_audience`）

## Known limitations

1. **评论 ≠ 全部受众**——留评论的是受众里最活跃的一小撮（沉默大多数不在数据里）。persona 偏向"会评论的人"，不是"所有看的人"
2. **平台评论可被污染**——水军 / 引战 / 跑题评论会进数据。hit-persona 聚类时对明显异常值降权，但不能完全过滤
3. **persona 滞后于真实受众变化**——画像基于过去 N 篇的评论。受众结构变了，要等新复盘累积才反映
4. **不解决"我想要的受众 ≠ 我实际的受众"**——persona 只如实报告"现在谁在看"。想转向另一种受众是选题战略问题，hit-persona 只提供"现状 vs 目标"的 gap，不替你做战略
