---
name: hit-migrate
description: 把老用户的 .hit-state.json 升级到当前 schema_version。读 migrations/registry.md 算迁移链，按顺序应用每一步迁移文件。幂等：跑两次结果一样。失败停在中间版本不前进。触发词："迁移"/"升级 state"/"migrate"/"我的 state 是老版本"/"schema 版本不对"。
argument-hint: [— from: <version>] [— to: <version>] [— dry-run]
allowed-tools: Bash(*), Read, Write, Edit, Skill
---

# /hit-migrate — Schema 版本迁移

把用户 `.hit-state.json` 从旧 `schema_version` 升级到 hit-lab 当前期望的 `LATEST_SCHEMA`。
...
[用户：迁移 / 或 SessionStart 提示后用户跑]
  ↓
[Phase 0: 读 .hit-state.json + migrations/registry.md → 确定迁移链]
  ↓
[Phase 1: dry-run（默认）展示迁移计划，等用户确认]
  ↓
[Phase 2: 备份 .hit-state.json → .hit-state.json.backup-<timestamp>]
  ↓
...
> 💡 调用时覆盖：`/hit-migrate — dry-run: false` 直接执行 / `/hit-migrate — to: 1.2` 仅升到指定版本
...
1. 读 `.hit-state.json` → 解析 `current_version = state.schema_version`
...
⚠️ 备份位置: .hit-state.json.backup-<timestamp>
...
```bash
cp .hit-state.json .hit-state.json.backup-$(date +%s)
```

输出："📦 备份到 .hit-state.json.backup-1714838400"
...
1. 输出 "→ [N/M] 应用 {file}..."
2. 读 `migrations/<file>` → 找到 `## HOW (Claude steps for /hit-migrate)` 段
...
5. 如某步失败：
   ...
   - 提示："已停在 schema_version: {last_success_version}。修复后重跑 /hit-migrate 会从这里继续"
   - 退出

### Phase 4: 验证

升完后：
1. 读 `.hit-state.json` → 解析 → 应能成功
...
📦 备份保留：.hit-state.json.backup-1714838400
   （确认一切正常后可手动 rm；下次成功 /hit-init 也会清理过期备份）
...
6. **不动 predictions / rubric / videos**：只改 `.hit-state.json`。其他用户数据由各自 skill 负责，迁移 skill 不碰
...
- 「在跑 hit-bump / hit-predict 中途调 migrate」 → 拒绝。等其他 skill 完成再跑，避免 in_progress_session 状态被破坏

---

## Integration

- 上游：SessionStart hook 检测 `state.schema_version != LATEST_SCHEMA` → 输出红色警告 + 建议跑 `/hit-migrate`
- 上游（手动）：用户 git pull 拉了新版后，看 CHANGELOG 标 BREAKING → 主动跑
- 下游：跑完后所有其他 skill 读 state 都能拿到最新字段
- 与 `hit-init`：init 写新 state 时直接用 LATEST_SCHEMA，不需要走 migrate
...
修复 .hit-state.json 后重跑 /hit-migrate 会从 1.2 → 1.3 继续。
```
