...
> 💡 调用时覆盖：`/hit-recommend — top: 3 — filter: safe`
...
| `candidates.md` | 用户项目根 |
| `predictions/*.md` | 用于去重 |
| `.hit-state.json` | 当前 rubric_version |
...
**无候选池引导**（核心：不让用户第一次遇到 hit-recommend 时被劝退）：
...
4. 📋 从 Notion / RSS 导入：跑 /hit-init --mode add-pool 配置 adapter

你也可以跳过候选池，直接给我具体稿子说"启动预测"。

> /hit-seed vs /hit-trends 的区别：
...
- 「把 candidates.md 里所有 entry 都重新打分一遍」 → 路由到 `/hit-score` 单条做；批量重打分是 `/hit-bump` 的一部分，不在 recommend 范围
...
- 上游：`/hit-trends` 把外部热点拉进 candidates.md → recommend 自动看到
- 下游：用户挑一条后写稿 → `/hit-predict`（candidate 的粗 composite 不进入 prediction，prediction 重新打）
- 与 `/hit-status` 协调：status 显示 "candidates 池有 N 条 tier1 未发"，recommend 提供具体推荐
