---
name: hit-shoot
description: 登记一条视频已拍摄。**建 video folder + 询问实际拍摄稿是否与 scripts/<id>.md 一致 + buffer +1**。与 hit-publish 配对：拍了进队列，发了出队列。触发词："拍了"/"拍了 X"/"shot"/"shot it"/"已拍 X"/"录完了"。
argument-hint: <scripts-path-or-id>
allowed-tools: Bash(*), Read, Write, Edit, Glob
---

# /hit-shoot — 登记拍摄完成 + 建 video folder + (改稿则) 触发 v2 预测

把视频从"已写预测、未拍摄"状态推进到"已拍摄、未发布"状态。这一步：
1. **建 `videos/<同 id>/`** 目录（之前没有的话）
2. **询问用户**："实际拍摄时用的稿子和 `scripts/<id>.md` 一致吗？"
3. 算 diff——超过 V2_TRIGGER_THRESHOLD (默认 30%) → **delegate 到 `/hit-predict — mode: v2`** 在原 prediction 文件 append `## 预测 v2` 段
4. 把 video folder 加进 state.shoots 队列，buffer +1

hit-shoot 自己**不**写预测内容——所有预测落盘逻辑在 hit-predict。hit-shoot 只负责检测改稿 + 派发。
...
3. 验证有对应 prediction `predictions/<同名>.md`：
   - 不存在 → **拒绝登记**，提示"先跑 /hit-predict 写预测，否则违反盲预测原则——你不能拍完才写预测，那等于事后看了画面写"
   - 存在 → 通过

### Phase 1：检查重复

读 `.hit-state.json`，检查 `shoots[]` 是否已含此 id：
- 已存在 → 警告"已登记过（X 天前）。是要重新登记，还是要用 /hit-publish 发布？"
- 不存在 → 进入 Phase 2
...
c) 大改了，基本是另一条 → 走 _redo 流程：
   scripts/<id>_redo.md → 重新 hit-predict → 再 hit-shoot（原 prediction 留档脱钩）
```
...
4. 提供了的话：算 diff
   ```bash
   # 解析 hit-lab 源码根（hit-shoot 是 symlink 装的）
   SKILL_REAL="$(readlink -f ~/.claude/skills/hit-shoot 2>/dev/null || readlink ~/.claude/skills/hit-shoot 2>/dev/null)"
...
5. **判定 v2 触发**：
   - `diff_pct >= 30` → 默认建议 v2 重判，**主动调用** `/hit-predict — mode: v2 — prediction-file: predictions/<id>.md` 传 `videos/<id>/script.md` 作 input。hit-predict 走 v2 模式 append `## 预测 v2`
   - `diff_pct < 30` → 询问用户："只改了 N% 的内容，要重判吗？默认不（v1 预测仍有效）"。用户说要 → 同上调用；用户说不 → 跳过 v2，继续 Phase 4
6. hit-predict 完成 v2 落盘后，控制权回到 hit-shoot 进 Phase 4

**c 路径（大改）**：
- 不写 `videos/<id>/script.md`，提示走 `_redo` 流程
- 退出 hit-shoot（不进 Phase 4）
...
`v2_prediction_written: true` 表示 prediction 文件里现在有 `## 预测 v2` 段，hit-retro 应读 v2 算偏差；`false` 表示沿用 v1。
...
1. **不写 prediction**——拍了 ≠ 发了。预测在 /hit-predict 锁，拍只是事件
...
- 「拍了 X，但我从来没跑过 hit-predict」 → 拒绝。v1 预测**必须拍前写**——拍完才写预测会被画面诱导事后修改。请先 /hit-predict 写 v1 再来 /hit-shoot。（v2 重判是另一回事——v1 已存在 + 拍后改稿才允许）
...
- 上游：`/hit-predict` 写完 prediction → 用户拍摄 → `/hit-shoot` 登记
- 下游：`/hit-publish` 发布时把对应项从 state.shoots 移除
- `/hit-status` 看板的 buffer 数字直接来自 `state.shoots.length`
- `/hit-recommend` 看 buffer 颜色调推荐策略
...
按 `shot_at` 升序——最早拍的在前面。`/hit-status` 显示最早一项的 days-since-shoot 警告（避免有视频拍了 30 天没发）。
