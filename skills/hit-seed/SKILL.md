---
name: hit-seed
description: 跟用户对话讨论选题——**默认一次一个**，用户主动给主题或经历，AI 围绕用户的输入深挖、提炼角度、写一份 draft。不是 AI 拿三个开放问题追用户，也不是一次给 5 个候选。触发词："找选题"/"我想做一条 X"/"最近有个想法"/"seed"/"启动种子"。可选 batch 模式：`/hit-seed --batch 5` 走旧的 brainstorm 5 候选 + 写 5 draft 流程。
argument-hint: [— batch: N] [— sources: <comma-separated>]
allowed-tools: Bash(*), Read, Write, Edit, Glob, WebFetch, Skill
---

# /hit-seed — 选题对话（默认）/ 批量 brainstorm（可选）

hit-seed 的核心是**跟用户讨论选题**，不是机械地 brainstorm。好内容来自用户的真实经历 + 观察 + 情绪——这些是 AI 不可能凭空 brainstorm 出来的。AI 的角色是**听用户讲 → 帮提炼角度 → 写一份 draft**，不是 dump 15 候选让用户挑。
...
Batch Mode — 用户显式要批量（`/hit-seed --batch 5`）：
...
| 必填 | 来源 |
|---|---|
| `.hit-state.json` | 读 calibration_samples / typical_duration / cadence |
...
| `audience.md`（如有） | 受众画像——选题 / 写稿时的"谁在看"镜子（由 `/hit-persona` 派生） |

## Workflow

### Phase 0: 前置检查 + 加载所有 context（**核心：3 个 context 来源**）

1. 读 `.hit-state.json` → 不存在则提示先跑 `/hit-init`
...
- **B 缺席**（benchmark 为空）+ 用户数据少：纯靠用户 input + 抓热点；明确告诉用户"没 benchmark 也没足够自己数据，建议跑 /hit-learn-from 后再回来 brainstorm"
...
注意这是**唯一的开放式问题**——只在用户**纯触发** `/hit-seed` 时问。如果用户已经在触发词里给了内容（"/hit-seed 我想做..." 或 "找选题 我最近开会..."），直接进 Mode A/B/C 不再问这一句。
...
详见 commit history（旧 hit-seed 的 Phase 1-3）。这是 escape hatch，不是默认。
...
模型的训练先验会把"视频脚本"默认写成提词器/字幕的短行格式。**这是错的**——hit-seed 的 draft 是给用户**改写**的散文稿，不是拍摄字幕。字幕是剪映拍后自动断的，不是写作时的形态。
...
>    - 加你的语气、个人经历、真实金句
>    - 砍铺垫、砍模型缩写、砍学术包装
> 2. 改完后跑 `/hit-predict scripts/<本文件>.md`
> 3. 拍完跑 `/hit-shoot scripts/<本文件>.md`
...
**为什么安全**（不污染校准）：hit-seed 的 draft 不是被预测/发布的东西——用户改写后、hit-predict 打分的是**用户最终稿**。这两步只是给用户更干净的起点。
...
用户说"今天就这样" → 结束 hit-seed。
用户给新 topic → 回 Phase 1 重新分流。

## Key Rules

1. **AI 不主动开放问**——只在用户纯触发 `/hit-seed` 时问一次入口问题，其他时候**等用户给输入再深挖**
...
- 上游：`/hit-init` Phase 5 末尾在 `pool_status=none + calibration_samples=0` 时主动询问"现在跑 /hit-seed？"
- 上游：`/hit-recommend` 在 candidates 空时引导文案中提及 `/hit-seed`
- 上游：`/hit-status` 在 `pool_status=none + 距 init >24h` 时提示"还没拍——跑 /hit-seed？"
- 下游：用户的 candidate → candidates.md（tier1，已 deep_read）
- 下游：（默认）draft → Phase 4.5 humanizer 去 AI 味 → scripts/<id>.md → 用户改写 → /hit-predict
- 可选依赖：[`humanizer`](https://github.com/blader/humanizer) skill（MIT，外部项目）。装在 `~/.claude/skills/humanizer/` 时 Phase 4.5 自动启用；未装则优雅跳过。**不打包进 hit-lab**——用户自己 clone
- 与 `/hit-trends` 区别：hit-seed 是**讨论 + 写 draft**（重 conversation）；hit-trends 是**多 adapter 抓 + 粗打分**（重 fetch）。两者目的不同，不互相替代。
