...
# /hit-predict — AI 主导的盲预测 + 用户 review

**这个工具的杀手锏是 AI 帮你做判断**。所以 hit-predict 的核心是：
...
[Phase 1: 读 script + rubric + state + 派生 confidence]
  ↓
[Phase 2: **委派 hit-score-blind sub-agent**（Task tool）拿 9 维盲打 + per-dim confidence]
  ↓
...
- **BLIND_SCORING = on**（默认）/ off —— 是否走 [hit-score-blind](../hit-score-blind/SKILL.md) sub-agent。off 等价于 `--skip-blind` flag，标 `last_prediction_self_scored: true` 给 hit-status 警告
...
> 💡 调用时覆盖：`/hit-predict scripts/<id>.md — BLIND_CHECK: lenient` / `--skip-blind`（都不推荐）
...
| 任意外部 .md 文件（如 `~/Desktop/my-draft.md`） | **警告 + 询问**："建议把稿子放到 scripts/<date>_<id>_<short>.md 让 hit-lab 管理。要我帮你 cp 过去并算 id 吗？"用户同意 → 建标准路径再继续 |
...
3. 读 `rubric_notes.md`，识别当前公式 + 维度（同 hit-score Phase 2）
4. 读 `.hit-state.json` 拿 `rubric_version`、`content_form`、`calibration_samples`、`typical_duration_seconds`、`baseline_plays`
...
### Phase 2: 委派 hit-score-blind sub-agent 拿盲打分

**BLIND_SCORING=on**（默认）—— 主 Claude 不再 inline 打分。通过 Task tool spawn `hit-score-blind`，让一个 context-isolated 的 sub-agent 只看 script + rubric_notes.md 给出 N 维分。

详见 [hit-score-blind/SKILL.md](../hit-score-blind/SKILL.md) 的"主 Claude 调用契约"段。**Task prompt 必须精简**：
...
**沙盒 escape**：`BLIND_SCORING=off` 或 `--skip-blind` —— 主 Claude 自己打 7 维。state 立刻标 `last_prediction_self_scored: true` + `last_self_scored_at: <ISO>`，hit-status 持续提示警告。仅用于：
...
3. **Refusal check**：`refusal != null` → 按 [hit-score-blind/SKILL.md](../hit-score-blind/SKILL.md) Phase 2 的处理表对应路径
...
（待填——T+RETRO_WINDOW_DAYS 天后跑 /hit-retro <对应 video folder>）
```
...
hit-retro 复盘时按"读最后一个 `## 预测 vN`"逻辑，自然取到 v2 算偏差。
...
`in_progress_session` 在 `hit-publish` 触发时清除。如果用户预测后从未 publish（弃稿），下次 `/hit-init` 或 `/hit-status` 检测到陈旧 in_progress 会询问是否清理。

`last_prediction_self_scored`：
- `true` 仅当本次预测走了 `--skip-blind` 或 Phase 2.5 用户选了 b（信主 Claude 自估）
- 一旦 `true` → hit-status 持续 nag："上次预测没走 blind sub-agent，已 N 天"——直到下次 normal `hit-predict`（走 sub-agent）触发后才清回 `false`
- `last_self_scored_at` 跟随更新；下次 `hit-predict` 走 sub-agent → 这两个字段一起被重置
...
- 「sub-agent 太慢，你直接打就行」 → 用 `--skip-blind` flag 显式声明。**不接受**主 Claude 自作主张跳过 sub-agent。flag 触发 state.last_prediction_self_scored=true，hit-status 持续提示直到下次 normal 调用清除
- 「Phase 2.5 选 b 后我不想标 last_prediction_self_scored=true」 → 拒绝。选 b 等于"我有意接受主 Claude 自评"——必须留下污染追踪轨迹
- 「我是 cold-start 但想跑完整版预测，给我 bucket 数字」 → 拒绝。前 5 篇 bucket 数字是 false precision，给反而误导。等第 5 篇复盘后 hit-status 会主动提示解锁。如果用户确实想要数字（罕见，自我教育目的）→ 允许但在文件头醒目标 `**Numerical predictions in cold-start are NOT predictive — for self-education only**`

## Integration

- 前置：`/hit-init` 必须完成 + `rubric_notes.md` 存在
- 上游可选：`/hit-score` 反复尝试不同稿子版本
- 下游：`/hit-publish`（发布登记）→ `/hit-retro`（复盘）→ 累计 ≥ MIN_SAMPLES 后 `/hit-bump`
- hook 依赖：`hooks/prediction-immutability.sh` 必须已安装在用户 `.claude/settings.json`，否则 immutability 仅靠 SKILL.md 自律——`hit-status` 会持续提示
