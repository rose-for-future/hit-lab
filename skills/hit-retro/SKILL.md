...
[Phase 6: 写入 rubric-memo.md 的"观察记录"段]
  ↓
[Phase 7: 检测是否触发 bump 候选 → 提示用户跑 /hit-bump]
```
...
- **AUTO_PROPOSE_BUMP = true** — Claude 判断是否系统性偏差时自动提议 /hit-bump
...
> 💡 调用时覆盖：`/hit-retro <file> — window: 7 — source: adapter`

## Inputs

| 必填 | 来源 |
|---|---|
| `<prediction-file>` 或 `<video-folder>` | 用户参数；缺失则从 `.hit-state.json` 的 `pending_retros[0]` |
| `rubric_notes.md` | 用户项目根（只读，用于当前规则上下文；不得写入实绩观察） |
| `rubric-memo.md` | 用户项目根（写入复盘观察、实绩证据、样本名与评论信号） |
| `.hit-state.json` | 状态文件 |

### 入参解析（同 hit-predict 双形态接受）
...
4. 校验文件 header 有 `Published at` → 没登记的不能复盘，提示用户先 `/hit-publish`
...
> `<adapters-dir>` = 克隆源码处的 `hit-lab/adapters/perf-data/`（install.sh **不**复制 adapter 到 ~/.claude/skills，只复制 15 个 skill）。定位：`find ~ -path '*/hit-lab/adapters/perf-data' -type d | head -1`。
...
- hit-retro 读这个 report.md 解析关键数据 → 摘要写入 prediction 的复盘段
...
Diff `scripts/<id>.md`（pre-shoot 草稿，可能是 hit-seed 写或用户写）vs `videos/<id>/script.md`（实际拍摄稿——hit-shoot 时用户提供的版本），找出**改动且对流量有明显影响**的部分：
...
如果 `videos/<id>/script.md` **缺失**（hit-shoot 时用户标 `script_lost`） → 跳过 4b，没法 diff。
...
**检测跨样本 pattern**：扫描 `rubric-memo.md` 已有"观察记录"，看新观察是否与某条已有观察形成 ≥2 样本支持。命中则在 `rubric-memo.md` 升级到"重大跨样本观察"段。只有在后续 `/hit-bump` 落地时，才把已验证的规律抽象成通用语言写入 `rubric_notes.md`。
...
读 `.hit-state.json` 的 `consecutive_directional_errors` 字段，按本次复盘判定向更新：
...
4. **不在复盘里 bump**。Phase 7 只**提议** bump，实际升级走 `/hit-bump`——避免一次操作做两件事
...
- 「直接 bump，不要单独走 /hit-bump」 → 拒绝。bump 流程有完整的跨模型审 + cleanup pass，retro 是触发器不是执行器

## Integration

- 前置：`/hit-publish` 已登记 + 时间窗口达到
- 下游：累计 `consecutive_directional_errors` 满 3 → 触发 `/hit-bump` 提议
- 状态字段更新：`calibration_samples` +1（这是 hit-status 显示进度的关键）
- pending_retros：剔除本条
