...
[用户：抓热点]
  ↓
[Phase 0: 读 .hit-state.json 拿 enabled adapters]
  ↓
...
[Phase 3: 去重（vs candidates / predictions / trends-history）]
  ↓
[Phase 4: 对每个新 item 粗打分（调 hit-score 内联逻辑）]
  ↓
...
- **DEDUPE = true** — 去重开关
- **AUTO_SCORE = true** — 抓回来后自动调 hit-score 粗打分
- **MIN_COMPOSITE_TO_SUGGEST = 6.0** — 低于此分的不推荐用户加入候选池（仍写入 trends-history 避免下次重复推）

> 💡 调用时覆盖：`/hit-trends — sources: manual-paste,hackernews,bilibili-popular — max-per: 10`

## Inputs

| 必填 | 来源 |
|---|---|
| `.hit-state.json` | 默认 sources |
| `adapters/trend-sources/<name>.md` | 各 adapter 的实现描述 |
| `candidates.md` | 去重对照 |
| `predictions/*.md` | 去重对照（已发的不再推） |
| `.hit-cache/trends-history.jsonl` | 历史抓取去重缓存 |
...
| `youtube-trending` | 需 API key 配置在 `.env` 或 .hit-state.json，调 YouTube Data API v3 `videos?chart=mostPopular` |
...
4. 检查 `.hit-cache/trends-history.jsonl` 已含此 id 且 `rejected_at != null` → 跳过
...
2. 按当前 rubric 给 7 维打分（**不**调 `/hit-score` 子 skill 走 IO；inline 复用打分逻辑）
3. 算 composite
4. 给一句 rationale

**注意**：粗打分 ≠ 正式预测。预测必须基于最终稿（用户改过的），这里的打分只是"是否值得展开写"的粗筛。

`AUTO_SCORE=false` 时，items 写入 candidates.md 时 composite=null，需要后续手动 `/hit-score`。
...
2. 所有抓回来的 items（不管选中与否）→ append 到 `.hit-cache/trends-history.jsonl`：
   ```jsonl
...
- 「跳过粗打分，直接写 raw 标题」 → 允许（`AUTO_SCORE=false`），但提示用户后续需要 `/hit-score` 才能进 recommend 池

## Integration

- 上游：用户配置 `.hit-state.json` 的 `enabled_trend_sources` 数组
- 下游：`/hit-recommend` 直接读 `candidates.md` 排序——trends 写完，recommend 立刻看到
- 与 `/hit-init`：onboarding Q4 选"没有候选池"的用户被引导到这里
- 与 `/hit-status`：status 看板显示"上次抓热点：X 天前 / 待清理候选池：Y 条"
