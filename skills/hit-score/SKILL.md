...
> 💡 调用时覆盖：`/hit-score draft.md — OUTPUT_DETAIL: compact`
...
1. 读 `.hit-state.json` → 不存在则提示用户先跑 `/hit-init`，停止
...
改成**通过 Task tool 调 `/hit-score-blind` sub-agent**，主 Claude 只做调度 + review。详见 [skills/hit-score-blind/SKILL.md](../hit-score-blind/SKILL.md)。
...
**禁止**塞进 Task prompt 的东西（[hit-score-blind/SKILL.md](../hit-score-blind/SKILL.md) 的"主 Claude 调用契约"段）：
- 用户对话引用 / 摘录
- 含播放数 / 万 / w / k 等字眼
...
- ❌ 给 bucket 概率分布（那是 hit-predict 的活）
- ❌ 触发"已发布"或"复盘"逻辑
- ❌ 提议 rubric 升级（即使打分时发现明显异常也只在控制台提示，不动 rubric）

## Key Rules

1. **打分走 sub-agent**。主 Claude 不再 inline 打分。看 [hit-score-blind/SKILL.md](../hit-score-blind/SKILL.md) 的隔离协议
2. **整数分**。不允许 4.5、3.7
...
6. **不算 candidate composite**。candidates.md 里的 composite 字段在 hit-trends/hit-recommend 里写——score 只服务"已写好的具体稿子"

## Refusals

- 「打分顺便预测一下」 → 拒绝。请改用 `/hit-predict`。原因：predict 必须走 blind check + 写 immutable 日志，score 跳过这些
...
- 「跳过 blind sub-agent 让主 Claude 直接打」 → hit-score **不接受**这种 escape hatch（与 hit-predict 不同；hit-predict 有 `--skip-blind`）。score 是轻量探索，没理由放弃隔离。如真的 Task tool 不可用 → 提示用户配置后再试

## Integration

- 是 `hit-predict` 的前置探索：用户可以反复 score 不同稿子版本，确定一份再 predict
- score 不更新 `.hit-state.json`——这是无副作用操作
