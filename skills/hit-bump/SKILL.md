...
- **READINESS_HEURISTIC** —
  ...
  - **写在 prediction header 或 hit-bump 输出时必说明**：本次提议是 default-aligned 还是 judgment-driven，给用户审视依据
...
**bump 是工具最高风险动作——所有重打必须走 [hit-score-blind](../hit-score-blind/SKILL.md) sub-agent**。inline 重打 = 主 Claude 已经看过实绩，rank 一致性变成 overfit 而非真信号。

#### 强制约束

- **不接受 self-scored fallback**——`/hit-predict` 有 `--skip-blind` flag，但 `/hit-bump` **没有**。如果 Task tool 不可用 → **abort bump**，向用户报告"先解决 Task tool 再 bump"
...
3. **通过 Task tool spawn hit-score-blind sub-agent**：
   ```
   ...
   Input:
     ...
     sidecar_path: .hit-cache/bump-rescores/<prediction-id>.json
...
5. 写"重打表"到 `.hit-cache/bump-rescores.json`（汇总）。**每条 entry 标 `blind: true`** —— bump phase 5 cleanup 时把这个字段连同新分数写到 prediction 文件的 `Re-scored under v<N+1>` 行
...
| Task tool 整个不可用 | abort bump，提示用户"Task tool 是 bump 的硬依赖。如真的离线环境，跑 `/hit-bump --bucket-only` 走轻量分支" |
...
- 仅依赖本地判定
- state file 持续标记，hit-status 持续提示用户"这次 bump 是自审，建议配置 mcp__llm-chat__chat"
...
（rubric bump 时全量重算，由 hit-score-blind sub-agent 独立打分；详见 rubric-memo.md 的 v2 → v2.1 升级 Memo）
```
...
`/hit-bump --bucket-only [--scheme ratio|absolute|percentile]`
...
2. 更新 `.hit-state.json` 的 `baseline_plays` 字段（bucket scheme 不持久化——下次 hit-predict 实时派生）
...
下一次 `/hit-predict` 起按新 bucket 派生。历史 prediction 文件里的 bucket 标签**不重算**——bucket 是预测时的语义判断，事后改写会破坏盲度。
...
- 「跳过 hit-score-blind sub-agent，主 Claude 直接重打就行」 → 拒绝。bump **不接受**任何 self-scored fallback——sub-agent 不可用 → abort bump，不接受"自审"
...
- 上游：`/hit-retro` 检测到 ≥3 同向偏差 → 提议跑 `/hit-bump`
- 依赖：`mcp__llm-chat__chat`（如配置）+ Task tool（spawn hit-score-blind）
- 修改：
  - `rubric_notes.md`（结构性更新，**绝不**写真实视频名 / 实绩）
  - `rubric-memo.md`（**新**——append Memo 全文，含证据 + 派生证据）
  - 所有 `predictions/*.md`（追加 Re-scored 行，不动预测段）
  - `.hit-state.json`
- 下游：下一篇 `/hit-predict` 自动按新 rubric_version 打分
