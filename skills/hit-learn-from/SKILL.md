...
# /hit-learn-from — 对标账号导入

工具早期最重要的信号源是**对标账号**——你 init 完没数据，rubric 等权 v0 等于占星。但如果你能找一个你想做成那样的账号，导入 5-10 条它的高/中/低样本，工具就有了 anchor。

后期当你自己 calibration_samples ≥ 10 时，benchmark 影响自然减弱——你的真实数据成为主要信号源。但 benchmark.md **不删**，仍是 hit-seed brainstorm 的 reference frame。

## Overview

```
[用户：学这个账号 / 启动 hit-learn-from]
  ↓
...
读 `.hit-state.json` 的 `benchmark_status`：
...
- 「能不能不写 transcript 文件，只在内存里拆」 → 不行。transcript 持久化是后续 hit-retro Phase 4b diff 的依据
- 「帮我下载对标视频」 → 拒绝。引导用户用 yt-dlp / BBDown 等工具自己下

## Integration

- 上游：`/hit-init` Phase 2.5 在 cold-start 用户时**强烈建议**跑 `/hit-learn-from`；calibration 用户**可选**
- 上游：`/hit-status` 在 `benchmark_status=pending` + 距 init >24h 时持续提醒
- 下游：`/hit-seed` brainstorm 时读 benchmark.md → 知道用户对标方向
- 下游：`script_patterns.md` 加新段，hit-seed 写 draft 时按 pattern 选结构
- 下游：`rubric_notes.md` 加 benchmark-derived signals 段，hit-bump 时作为参考之一
- N≥10 提示：`hit-status` 在用户 calibration_samples ≥10 时提示"你已有足够自己数据，benchmark 影响淡出（保留作 sanity check）"
...
判断条件 + 默认值都在 hit-status 触发器 #19 / hit-seed Phase 0 里实现。

**淡出后**：
- hit-seed brainstorm 仍读 benchmark.md，但**优先级低于用户自己的 predictions/**
...
**任何时候用户主动**：跑 `/hit-learn-from --replace none` 完全解除 benchmark 影响

## 与其他 skill 的区别

| Skill | 用途 |
|---|---|
| `/hit-learn-from` | **从对标账号**导入 pattern / rubric 信号（一次性 / 偶尔追加） |
| `/hit-seed` | brainstorm 选题 + 写 draft（读 benchmark.md 作参考） |
| `/hit-trends` | 抓今天的热点（与 benchmark 无关） |
| `/hit-bump` | 升级 rubric（用户自己 N≥5 后用真实数据，不直接用 benchmark signals） |
