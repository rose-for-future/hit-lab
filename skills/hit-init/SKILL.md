---
name: hit-init
description: hit-lab 的首次 onboarding 与脚手架创建器。统一流程——所有用户都走相同 5 阶段闭环，唯一区别是"发过视频的人"会在 init 时多一步：抓取已有视频建立历史 context（用于后续 hit-seed 给更贴合的选题、更准的 baseline）。触发词："初始化"/"init"/"首次使用"/"我是新用户"/"setup hit-lab"。**必须在用户第一次会话执行；其他子 skill 在 .hit-state.json 不存在时自动路由到此。**
argument-hint: [— form: opinion-video|long-essay|short-text|podcast]
allowed-tools: Bash(*), Read, Write, Edit, Glob, WebFetch, Skill
---

# /hit-init — 首次 onboarding
...
1. 读用户当前工作目录（**用户的 content project，不是 hit-lab 自己**）
2. 检查是否已存在 `.hit-state.json`：
...
不再因为 content_form 拒绝继续——任何形态都允许，只是 `rubric_form_mismatch` 字段标真，hit-status 后续会持续提示用户"你的形态需要 bump 调权重"。
...
>    第一次 publish 之前装上就行（hit-status 会持续提醒装）。
...
> a) 没有（默认）— 一会儿我帮你 brainstorm，或日常用 /hit-trends 抓
> b) 有，markdown 列表
> c) 有，Notion / 其他"
...
a) 现在找 → 立刻进入 /hit-learn-from（5-15 分钟，看你材料准备程度）
b) 等下找 → state 标 `benchmark_status: pending`，hit-status 持续提醒
...
行为：
- 选 a → Phase 3 创建脚手架完毕后，**自动 dispatch 到 /hit-learn-from**（不让用户手动跑——已经在 init 流程里了）。完成后回 init Phase 4
- 选 b → state 标 `benchmark_status: pending` + `benchmark_name: null`
- 选 c → state 标 `benchmark_status: none`

记录到 `benchmark_status` / `benchmark_name`（如 a 选则在 hit-learn-from 里写入）。
...
1. **`.hit-state.json`**
   ```
   "正在创建 .hit-state.json — 各子 skill 共享上下文的地方。
...
2.5. **`rubric-memo.md`**（**新**——配合 hit-score-blind 隔离协议）
   ...
   "正在创建 rubric-memo.md — bump 升级 Memo 累积档案。
    这是 hit-bump Phase 5 写入 Memo 全文（含真实视频名 + 实绩 + 派生证据）的位置。
    ...
    现在是空的，等第一次 hit-bump 升级后 append 第一段 Memo。"
   ```
   - 复制 `hit-lab/templates/rubric-memo.template.md` → `<user-repo>/rubric-memo.md`
...
3. **`script_patterns.md`**
   ...
   ```
   - 复制 `hit-lab/templates/script_patterns.template.md`

4. **四个目录**：`scripts/` + `predictions/` + `videos/` + `samples/`（都加 `.gitkeep`）
   ```
   "正在创建四个目录：
    
    scripts/      — 拍前的草稿（hit-seed 写或你写）
    predictions/  — immutable 预测日志（hook 保护）
    videos/       — 拍后的工作目录（hit-shoot 创建子目录）
    samples/      — 对标账号视频 / 转录（hit-learn-from 创建子目录）
...
4.5. **`benchmark.md`**（仅 Phase 2.5 选 a/b 时）
   ```
   "正在复制 benchmark.md 占位模板（实际内容由 hit-learn-from 填）—— 
    这是你的对标账号的中央 reference。
    前期工具的 rubric / pattern / 选题方向感大量从这里推；
    后期 N≥10 后影响淡出，但保留作 sanity check。"
   ```
   - 复制 `hit-lab/templates/benchmark.template.md` → `<user-repo>/benchmark.md`
...
4.7. **`audience.md`**
   ...
   "正在创建 audience.md — 你账号的受众画像（'谁在看'）。
    ...
    audience 告诉 Claude 你的观众是谁。跑够几篇复盘后跑 /hit-persona，
    它会从评论数据聚类出真实画像，hit-seed 选题写稿时就有了一面镜子。
    
    注意：audience.md 由评论派生 → 含实绩信号 → blind 打分 sub-agent 硬禁读它。"
   ```
   - 复制 `hit-lab/templates/audience.template.md` → `<user-repo>/audience.md`
   - 如 Phase 2.5 选了 a/b（有 benchmark）→ Phase 5 清单里提示"可跑 `/hit-persona — seed-from-benchmark` 先 seed 一份未验证画像"
...
6. **如果 Q5=是 → 安装 hooks**
   ...
   - 复制 `prediction-immutability.sh` + `session-start.sh` + `log-event.sh` 到 `.hit-hooks/`，chmod +x
   - settings.json 里的 command 路径用 `${CLAUDE_PROJECT_DIR}/.hit-hooks/`

7. **(Pool 选项 c—Notion)** 仅记录到 state file 的 `pool_status: notion`，后续 hit-trends 调用时再处理
...
- 派生 confidence 等级 → 后续 hit-predict 写预测时直接用
...
### Phase 4.5: 如 Phase 2.5 选 a → dispatch 到 /hit-learn-from

如果用户在 Phase 2.5 选了 a（现在导对标账号）→ **自动触发 /hit-learn-from**：

```
✅ 脚手架 + hooks 装完。

下面立刻进入 /hit-learn-from 帮你导入对标账号——
你 init 时选了"现在找"，不让你又开一个会话才跑。

[invoke /hit-learn-from]
```

hit-learn-from 完成后回到 init 的 Phase 5。
...
- 「我已经在别处初始化过了，把那个项目的配置同步过来」 → 慎重。提示用户手动 cp 现有 `.hit-state.json` 和 `rubric_notes.md`，不自动跨项目同步
- 「不装 hook 但保留 immutability 承诺」 → 允许，state 标 `hooks_installed: false`，hit-status 持续提示"你的 immutability 是君子协定"

## Integration

- 写完后，主 SKILL.md 的路由就解锁了所有其他子 skill
- `hit-status` 读 `.hit-state.json` 的 `calibration_samples` 字段决定显示哪个 confidence 等级
- 如 Q2=b 走了 import → 历史 reconstructed predictions 进 `predictions/` 和 `videos/<...>/`，但**不**计入 calibration_samples（不是真校准样本）
- `/hit-seed` 读 `predictions/` 的所有历史 reconstructed prediction → brainstorm 时知道"用户过去做过什么"
