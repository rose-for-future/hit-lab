...
被 `/hit-retro` 在 `state.data_collection=adapter` + `platform=douyin` 时自动调用。
...
输出写到**你的内容项目** `videos/<...>/report.md`（`hit-retro` 读这个文件 → 摘要写到 prediction 复盘段）。
调试产物（URL dump / 截图）写到 `.hit-cache/douyin-session-debug/`，避免散落在 skill 源码目录。
...
# ADAPTER="$HOME/Desktop/hit-test/hit-lab/adapters/perf-data/douyin-session"
...
hit-retro 自动调用，你不需要手动跑。但如果想手动测试：
...
- `https://v.douyin.com/abc123` → 短链需要 resolve（hit-publish 自动做）

hit-publish 会在登记发布时把 aweme_id 存到 prediction header（如能 resolve）。hit-retro 启动时直接读这个字段。
...
- `.hit-cache/douyin-session-debug/` 也不应提交到 git：里面可能含调试截图和接口 URL dump
