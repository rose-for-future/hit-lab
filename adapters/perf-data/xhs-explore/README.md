...
被 `/hit-retro` 在 `state.data_collection=adapter` + `platform=xhs` 时自动调用。
...
调试产物（URL dump / 截图 / galaxy 原始 JSON）写到 `.hit-cache/xhs-explore-debug/`。
...
# 4. 首次扫码登录小红书创作者中心
ADAPTER=$(find ~/hit-lab -name "xhs-explore" -type d 2>/dev/null | head -1)
python "$ADAPTER/crawler.py" login
# → 弹出 Chromium 窗口，用小红书 App 扫码
# → 登录成功后窗口自动关闭，cookie 存在 当前目录/.auth-xhs/
```

> Windows 提示：adapter 在 `~/hit-lab/adapters/perf-data/xhs-explore`（克隆源码处），
...
- `https://xhslink.com/xxxxx` → 短链，hit-publish 会 resolve

hit-publish 登记发布时把 note_id 存到 prediction header，hit-retro 启动时读这个字段。
...
| 笔记列表为空 | 创作者中心改了 galaxy 接口路径 | 看 `.hit-cache/xhs-explore-debug/creator_urls.txt`，更新 `GALAXY_*_KEYS` |
| 评论抓不到 | xsec_token 缺失 / 评论被关 | 看 `frontend_urls.txt`；拿不到就降级 manual 粘评论（hit-retro 会提示） |
...
1. **不要用 CDP/Playwright 主动 `Page.navigate` 刷新创作者后台**——实测连续两次自动跳转后，会话被风控直接 401 作废，被踢回扫码页（用户需重新扫码）。
2. **headless 模式同样会被踢**：即使带完整登录态，`--headless=new` 打开创作者中心也会 401。必须真实窗口。
3. **正确姿势**：真实窗口停在笔记管理页 → 被动 `Network` 监听页面自然发出的请求；需要翻页/补数据时，在**页面上下文内**跑 same-origin `fetch('/api/galaxy/...', {credentials:'include'})`，不要导航。
4. **`/api/galaxy/v2/creator/note/user/posted?tab=0` 只回部分笔记**——DOM（笔记管理列表）才是全量。抓全量历史时以 DOM 文本为准、API 补充字段。
...
- `.hit-cache/xhs-explore-debug/` 也不应提交（含调试截图 / 接口 URL / 原始 JSON）
