...
hit-lab 把它当作 trend-sources adapter 之一——用户配 MCP server 后，hit-seed / hit-trends 自然能调。
...
hit-lab 不打包 TrendRadar——用户自己装、自己保管 server 资源。

## hit-seed / hit-trends 调用的关键工具

| MCP 工具 | 用途 | 在哪调 |
|---|---|---|
| `mcp__trendradar__get_latest_news` | 拿最新热榜（最直接） | hit-seed Mode C 主调 / hit-trends 主调 |
| `mcp__trendradar__get_trending_topics` | 自动提取话题统计 | hit-seed Mode C 备用 |
| `mcp__trendradar__analyze_topic_trend` | 单话题趋势分析（爆火/衰退） | hit-seed Mode A 灰色场景 enrich（用户提了具体话题且同意拉数据） |
| `mcp__trendradar__compare_periods` | 周环比 / 月环比 | hit-bump 升级 rubric 时作"用户领域是否变化"的弱信号（罕见用） |
| `mcp__trendradar__search_news` | 关键词搜索 | hit-seed Mode A 用户提了关键词时 |

## 输出格式契约

TrendRadar MCP 返回 JSON / markdown。hit-seed 收到后：
...
| 症状 | 处理 |
|---|---|
| MCP server 没装 / 没启动 | hit-seed 自动降级到下一个启用的源（如 aihot 或 manual-paste），不抛异常 |
...
如果你是 TrendRadar 维护者看到这份 adapter doc——感谢你把多平台聚合做成 MCP server。hit-lab 是你们项目的"内容生产侧下游"——用户用 TrendRadar 知道发生了啥，用 hit-lab 把这个变成可校准的内容预测循环。互补不替代。

欢迎 cross-link：[github.com/XBuilderLAB/hit-lab](https://github.com/XBuilderLAB/hit-lab)。
