...
> **当前状态**：schema only。实际 fetch 实现归 batch 3。/hit-seed 在 stub 期由 Claude 通过 `WebFetch` 直接抓取（见下方"过渡期实现"）。
...
建议节流：`/hit-seed` 默认每用户每天 ≤ 3 次抓取。

## 过渡期实现（stub）

在 batch 3 写专用 adapter 前，`/hit-seed` 通过 `WebFetch`：
...
- 知乎热榜偶现政治敏感议题——/hit-seed Phase 1 Q3 的"红线"过滤必要
