...
## hit-seed / hit-trends 怎么调

**不要直接 curl** —— 让 Claude 自然触发 aihot skill 即可：

| hit-seed 场景 | 给 Claude 的内部指令 |
|---|---|
...
aihot skill 的 SKILL.md 已经详尽描述了端点 + 路由优先级（默认走精选不走日报）——hit-seed 不需要重复写这套逻辑，**信任 aihot skill 自己的判断**。

## 输出格式契约

aihot skill 默认返回 markdown，按 5 类（模型/产品/行业/论文/技巧）分组。hit-seed 收到后：
...
| 用户的 content_form 跟 AI 完全无关（如美食/妆教） | hit-seed 应该**不调 aihot**——按 [data-source-routing.md](../../shared-references/data-source-routing.md) 的路由表 |
