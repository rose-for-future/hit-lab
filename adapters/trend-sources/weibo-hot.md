...
被以下 skill 调用：`hit-seed` Phase 2a、`hit-trends`。

> **当前状态**：schema only。实际 fetch 实现归 batch 3。/hit-seed 在 stub 期由 Claude 通过 `WebFetch` 直接抓取 + 解析（见下方"过渡期实现"）。

---

## 适用场景

- **`hit-seed` 默认 source 之一**——cold-start 用户的第一次选题种子
- **`hit-trends` 可选 source**——日常补充候选池
...
- 其他字段：null（抓取阶段不打分；由调用方 hit-score 处理）
...
**优雅降级**：单次失败不抛异常——调用方（hit-seed / hit-trends）会用其他 sources 兜底。

## 稳定性等级

★★★ — 公开端点，但微博偶尔调整页面结构 + 有反爬（同一 IP 短时间高频抓取会被限流）。

建议节流：`/hit-seed` 默认每用户每天 ≤ 3 次抓取——cold-start 阶段不需要更高频。

## 过渡期实现（stub）

在 batch 3 写专用 adapter 实现前，`/hit-seed` 在调用本 source 时直接由 Claude 通过 `WebFetch` 工具抓 `https://s.weibo.com/top/summary?cate=realtimehot`，从 HTML 中提取 top 50 热搜标题。具体由 hit-seed 的 Phase 2a 处理：
...
- 微博热搜内容**经常含政治敏感 / 娱乐八卦**议题——/hit-seed Phase 1 Q3 的"红线"过滤至关重要
