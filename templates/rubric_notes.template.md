...
> **本文件是 hit-lab 评分规则进化的载体**。每次复盘实际播放数据 vs 预测分数后，把判断依据和规律显式写在这里，下次打分前 `/hit-score` `/hit-predict` 会先读这个文件再动手。 <!-- hl-ex -->
>
> **核心原则**：规律必须可追溯到具体样本。不写"情感共鸣很重要"这种空话，要写"XX 这篇 ER=5 得到验证 / 推翻，因为评论区 top 3 都是 YY 模式"。
>
> 完整生命周期协议见 [shared-references/observation-lifecycle.md](../hit-lab/shared-references/observation-lifecycle.md)。
> 升级流程见 [shared-references/bump-validation-protocol.md](../hit-lab/shared-references/bump-validation-protocol.md)。
...
**迁移触发**: 候选筛选时如遇旧版打分的文章进入 top → 当场重读重评；不做全量重评。**校准池（带实绩数据）必须在每次升级时全量重打**。 <!-- hl-ex -->
...
> Cold-start 用户应该等权起步——见 [opinion-video-zero.md](../hit-lab/starter-rubrics/opinion-video-zero.md)。
...
> - 实绩：播放 / 点赞 / 评论 / 转发（带 T+Nd 标注） <!-- hl-ex -->
> - Top 评论关键词：[简短摘录 + 赞数]
> - 判断：哪个维度被验证 / 推翻？为什么？
...
> ```
>
> 删除规则见 [shared-references/observation-lifecycle.md](../hit-lab/shared-references/observation-lifecycle.md)：被吸收为维度 → 删；被推翻 → 删。git history 是档案。

### 示例条目（来自视频分析项目，仅供参考；你的项目的真实条目从复盘后开始累计）

#### 2026-04-24 停止期待 (ab61ed09) — 验证情感向爆款【T+7d 数据】
- 预测：composite=8.24（v2: ER5/HP5/QL5/NA3/AB5/SR2/SAT4），bucket=30-100w <!-- hl-ex -->
- 实绩：T+7d 71.1w（中枢 50w，**+42%**），分播比 **2.53%** <!-- hl-ex -->
- Top 评论关键词：「她不一样」/「他不一样」全文出现 12+ 次变体（最高 2266 赞）
- 判断：ER=5 主导被强证据验证（与同 composite 谁问你了 11.7w 比，**6.07x 流量比**） <!-- hl-ex -->
...
> ⚠️ **bucket 边界是用户账号的属性，不是普适常量**——绝对数桶（"5w 是底部"）只对有粉丝基础的老手成立，对 0 粉新人会让所有视频都落"底部 99%"，bucket 失去排序意义。 <!-- hl-ex -->
>
> 本工具按校准阶段切换三种 bucket 方案。当前生效方案由 `.hit-state.json` 的 `bucket_scheme` 字段决定。

### 阶段 1：cold-start，比率桶（当前阶段）

`bucket_scheme = "ratio"`

**第 1 篇**：用平台通用默认（实际播放数） <!-- hl-ex -->

| Bucket | 范围（实际播放）| 含义 | 先验概率 | <!-- hl-ex -->
|---|---|---|---|
| 底部 | < 100 | 几乎被算法埋了 | 30% |
...
**第 2 篇起**：`baseline = 上一篇实际播放数`（或最近 3 篇中位数） <!-- hl-ex -->
...
详见 [starter-rubrics/opinion-video-zero.md](../hit-lab/starter-rubrics/opinion-video-zero.md) 的"比率桶方案"段。

### 阶段 2：N=5 后切到固定绝对桶（带 ratio 备用）

`bucket_scheme = "absolute_with_ratio"`

跑完 5 篇后，`/hit-bump --bucket-only` 自动派生：
- `baseline = 5 篇实际播放的中位数` <!-- hl-ex -->
- 边界 = baseline × {0.3 / 1 / 3 / 10 / 30}

`/hit-bump --bucket-only` 落地时会替换本段表格。
...
`/hit-status` 在 N=10 时主动建议切换。这种方案永远自洽——不管账号多大，"top 5%"语义稳定。

---

> **参考博主的绝对桶**（25+ 视频拟合，**只适用于已有粉丝基础的成熟博主**——你 calibrated 之前不要照搬）：
>
> | Bucket | 范围（万播放） | 先验概率 | <!-- hl-ex -->
> |---|---|---|
> | 底部 | <5w | 5% | <!-- hl-ex -->
> | 基础盘 | 5-30w | 35% | <!-- hl-ex -->
> | 命中 | 30-100w | 45% | <!-- hl-ex -->
> | 爆款 | 100-150w | 12% | <!-- hl-ex -->
> | 现象级 | >150w | 3% | <!-- hl-ex -->
