...
> **本模板由 `/hit-predict` 自动填充**。
> 所有预测都用统一格式（7 个组件 + 复盘段）——前 5 篇和成熟期都一样，区别仅是 header 里的 confidence 等级。
>
> 完整规范见 [shared-references/prediction-anatomy.md](../hit-lab/shared-references/prediction-anatomy.md)。
>
...
**Script Hash**: `sha256:<12 位>` (predict 时 hash script 内容；hit-shoot 时再 hash，不一致则复盘段加 integrity warning)
...
> 如用户从零写没用 hit-seed，写"用户原创稿，无 Claude 草稿对照"。
...
> ⚠️ **以下段落由 `/hit-retro` 在 T+`RETRO_WINDOW_DAYS` 天后追加**。
